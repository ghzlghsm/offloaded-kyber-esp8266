...
/*bool MqttAdapter::_rxq_push(const char* topic, const uint8_t* payload, uint16_t len) {
  MQTT_CRITICAL_START();
  uint8_t next = (uint8_t)((_rxh + 1) % MQTT_RXQ_N);
  if (next == _rxt) { MQTT_CRITICAL_END(); return false; }

  MqttRxItem& it = _rxq[_rxh];
...
/*bool MqttAdapter::popRx(MqttRxItem& out) {
  MQTT_CRITICAL_START();
  if (_rxt == _rxh) { MQTT_CRITICAL_END(); return false; }
  out = _rxq[_rxt];
  _rxt = (uint8_t)((_rxt + 1) % MQTT_RXQ_N);
  MQTT_CRITICAL_END();
  return true;
}*/
...
/*bool MqttAdapter::enqueue(const char* topic, const uint8_t* payload, uint16_t len, uint8_t qos, bool retain) {
  ...
  MQTT_CRITICAL_START();
  uint8_t next = (uint8_t)((_txh + 1) % MQTT_TXQ_N);
  if (next == _txt) { MQTT_CRITICAL_END(); return false; }

  MqttTxItem& it = _txq[_txh];
  strncpy(it.topic, topic, sizeof(it.topic)-1);
  it.topic[sizeof(it.topic)-1]=0;
...
bool MqttAdapter::enqueue(const char* topic, const uint8_t* payload, uint16_t len, uint8_t qos, bool retain) {
  ...
  _txh = next;
   ...
   // --- Kyber TX pacing (stability) ---
  // فقط برای topicهای kyber/.../in/... (data-plane)
  /*static uint8_t kyber_enq_cnt = 0;
  if (strncmp(topic, "kyber/", 6) == 0 && strstr(topic, "/in/") != nullptr) {
    delay(0);                 // yield کوتاه
    kyber_enq_cnt++;
    if ((kyber_enq_cnt & 0x03) == 0) {   // هر 4 تا enqueue
      delay(1);               // ریلکس واقعی (WDT + WiFi)
    }
  }*/
...
/*void MqttAdapter::pumpTx() {
  ...
  while (_inflight < MAX_INFLIGHT) {
    ...
    uint16_t pid = client.publish(it->topic, it->qos, it->retain != 0, (const char*)it->data, it->len);
    if (pid == 0) break;
