...
int PQCLEAN_MLKEM512_CLEAN_crypto_kem_enc_derand(uint8_t *ct,
    ...
    if (PQCLEAN_MLKEM512_CLEAN_indcpa_enc_ex(ct, buf, pk, kr + KYBER_SYMBYTES) != 0) {
        memset(ct, 0, KYBER_CIPHERTEXTBYTES);
        // ✅ FIX: ss طولش KYBER_SSBYTES هست
        memset(ss, 0, KYBER_SSBYTES);
        return -1;
    }

    // ✅ FIX: مثل PQClean رفرنس در حالت موفق ss را ست کن
...
int PQCLEAN_MLKEM512_CLEAN_crypto_kem_enc(uint8_t *ct,
    ...
    // ✅ FIX: کد خطا را propagate کن تا در app بفهمی offload fail شده یا نه
    return PQCLEAN_MLKEM512_CLEAN_crypto_kem_enc_derand(ct, ss, pk, coins);
}
...
int PQCLEAN_MLKEM512_CLEAN_crypto_kem_dec(uint8_t *ss,
    ...
    delay(0);
    if (PQCLEAN_MLKEM512_CLEAN_indcpa_enc_ex(cmp, buf, pk, kr + KYBER_SYMBYTES) != 0) {
        fail = 1; // اگر نتونستیم cmp بسازیم، به چشم fail نگاه کن
    } else {
