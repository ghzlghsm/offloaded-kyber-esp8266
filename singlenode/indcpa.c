...
// keypair_derand big locals -> globals
static polyvec g_a[KYBER_K];
static polyvec g_e, g_skpv2, g_pkpv2;
// ===== stack-safe globals for enc/dec =====
static polyvec g_at[KYBER_K];

static polyvec g_sp, g_ep, g_b;
static polyvec g_skpv_dec;          // جدا از g_skpv2 که برای keypair داری (ایمن‌تر)
static poly    g_v, g_k, g_epp, g_mp;
...
void PQCLEAN_MLKEM512_CLEAN_indcpa_keypair_derand(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
  ...
  // ---- use globals instead of stack locals ----
  polyvec *a    = g_a;        // polyvec[KYBER_K]
  polyvec *e    = &g_e;
  polyvec *pkpv = &g_pkpv2;
  polyvec *skpv = &g_skpv2;

  memcpy(buf2, coins, KYBER_SYMBYTES);
  buf2[KYBER_SYMBYTES] = KYBER_K;
  hash_g(buf2, buf2, KYBER_SYMBYTES + 1);

  // gen_matrix already stack-safe in your code (g_gen_buf)
...
void PQCLEAN_MLKEM512_CLEAN_indcpa_enc(uint8_t c[KYBER_INDCPA_BYTES],
    ...
    // --- use globals (no big stack locals) ---
    polyvec *sp   = &g_sp;
    polyvec *pkpv = &g_pkpv2;     // چون تو keypair اینو داری؛ اگر g_pkpv خواستی، اینو تغییر بده
    polyvec *ep   = &g_ep;
    polyvec *at   = g_at;         // polyvec[KYBER_K]
    polyvec *b    = &g_b;
...
void PQCLEAN_MLKEM512_CLEAN_indcpa_dec(uint8_t m[KYBER_INDCPA_MSGBYTES],
    ...
    polyvec *b    = &g_b;         // reuse
    poly   *v     = &g_v;         // reuse
    polyvec *skpv = &g_skpv_dec;  // dedicated
    poly   *mp    = &g_mp;
